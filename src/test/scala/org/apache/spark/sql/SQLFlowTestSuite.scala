...
class SQLFlowTestSuite extends QueryTest with SharedSparkSession {
  ...
  protected def runQueries(queries: Seq[String], testCase: TestCase): Unit = {
    ...
    val flowString = SQLFlow(GraphVizFormat).catalogToSQLFlow(localSparkSession)
    val contractedFlowString = SQLContractedFlow(GraphVizFormat).catalogToSQLFlow(localSparkSession)
