...
CREATE OR REPLACE TEMPORARY VIEW v16 AS
  SELECT count(t1a)
...
-- TC 02.07.01
-- set op
CREATE OR REPLACE TEMPORARY VIEW v20 AS
  SELECT t1a
  FROM   t1
  WHERE  t1a <= (SELECT   max(t2a)
                 FROM     t2
...
-- TC 02.07.02
-- set op
CREATE OR REPLACE TEMPORARY VIEW v21 AS
  SELECT t1a
  FROM   t1
  WHERE  t1a <= (SELECT   max(t2a)
                 FROM     t2
...
-- TC 02.09
-- in HAVING clause
CREATE OR REPLACE TEMPORARY VIEW v23 AS
  SELECT   t1a
  FROM     t1
  GROUP BY t1a, t1c
