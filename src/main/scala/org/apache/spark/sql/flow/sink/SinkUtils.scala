...
object SinkUtils extends Logging {
  ...
  def tryToExecuteCommand(cmd: String, arguments: String): Unit = {
    if (isCommandAvailable(cmd)) {
      val commands = Seq("bash", "-c", s"$cmd $arguments")
      try {
        BlockingLineStream(commands)
      } catch {
        case e =>
          logWarning(s"""Failed to execute `${commands.mkString(" ")}` """ +
