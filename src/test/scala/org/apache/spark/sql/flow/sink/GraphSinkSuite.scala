...
class GraphSinkSuite extends QueryTest with SharedSparkSession
  ...
  private def checkMermaidFormatString(actual: String, expected: String): Unit = {
    val edgePattern = """[a-zA-Z0-9_\.]-->[a-zA-Z0-9_\.]+"""
    // val edgePattern = """.+-->.+"""
    super.checkOutputString(edgePattern)(actual, expected)
  }
      ...
      withTempView("t2", "t3") {
        ...
        checkMermaidFormatString(flowString,
          """
            ...
            |    Aggregate_0a5906c(Aggregate)
            |    Filter_2219188(Filter)
            |    Project_b489723(Project)
            |    t3[["t3(sum,count)"]]
            |    default.t1[["default.t1(k,sum)"]]
            |    t2[["t2(k,sum,v2)"]]
            |    Aggregate_1f7cd44(Aggregate)
            |    t_54921ac[["t_54921ac(k,v)"]]
            |    Project_6795b79(Project)
            |    t_54921ac-->Aggregate_1f7cd44
            |    Aggregate_1f7cd44-->default.t1
            |    Project_6795b79-->Aggregate_0a5906c
            |    Filter_2219188-->Project_6795b79
            |    t2-->Filter_2219188
            |    Aggregate_0a5906c-->t3
            |    default.t1-->Project_b489723
            |    Project_b489723-->t2
          """.stripMargin)
        ...
        checkMermaidFormatString(contractedFlowString,
          """
            |flowchart LR %% Automatically generated by org.apache.spark.sql.flow.sink.MermaidSink
            |    t_8674461[["t_8674461(k,v)"]]
            |    t3[["t3(sum,count)"]]
            |    default.t1[["default.t1(k,sum)"]]
            |    t2[["t2(k,sum,v2)"]]
            |    t_8674461-->default.t1
            |    t2-->t3
            |    default.t1-->t2
          """.stripMargin)
      }
