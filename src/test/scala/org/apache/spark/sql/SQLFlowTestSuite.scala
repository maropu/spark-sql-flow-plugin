...
class SQLFlowTestSuite extends QueryTest with SharedSparkSession {
  ...
  protected def runQueries(queries: Seq[String], testCase: TestCase): Unit = {
    ...
    val graphFormat = GraphVizFormat(imgFormat = "svg")
    val flowString = SQLFlow(graphFormat).catalogToSQLFlow(localSparkSession)
    val contractedFlowString = SQLContractedFlow(graphFormat).catalogToSQLFlow(localSparkSession)
